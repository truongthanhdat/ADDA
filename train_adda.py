...
def main():
    ...
    source_ft, source_layers = model_fn(source_images, scope = "source_network", is_training = True,
                                        num_classes = source_dataset.num_classes, image_size = options.image_size)
    target_ft, targte_layers = model_fn(target_images, scope = "target_network", is_training = True,
    ...
    adversary_logits = adversarial_discriminator(adversary_ft, leaky = options.adversary_leaky)
    ...
    source_accuracy = slim.metrics.accuracy(tf.argmax(source_ft, axis = 1), tf.cast(source_labels, tf.int64))
    target_accuracy = slim.metrics.accuracy(tf.argmax(target_ft, axis = 1), tf.cast(target_labels, tf.int64))
    ...
    if options.solver == 'sgd':
        ...
        optimizer = tf.train.AdamOptimizer(learning_rate_op, 0.5)

    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope = "source_network")
    if update_ops:
    ...
    for iter in xrange(1, options.num_iters + 1):
        timer.tic()
        _mapping_loss, _adversary_loss, _, _, src_acc, tgt_acc, dom_acc, summary, lr  = sess.run([
                                                mapping_loss, adversary_loss,
                                                mapping_step, adversary_step,
                                                source_accuracy, target_accuracy, domain_accuracy,
                                                summary_op, learning_rate_op])
